...
char* indexes(const char* path)
{
    ...
    // make new path for index.php
    char* indexphp = malloc(strlen(path) + strlen(php) + 1);
    strcpy(indexphp, path);
    strcat(indexphp, php);
    
    // make new path for index.html
    char* indexhtml = malloc(strlen(path) + strlen(html) + 1);
    strcpy(indexhtml, path);
    strcat(indexhtml, html);
    
    // chek for exsisting index.php or index.html in the folder
    if(access(indexphp, F_OK) == 0) return indexphp;
    else if(access(indexhtml, F_OK) == 0) return indexhtml;
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *length = 0;
    ...
    int block;
    do {
        block = fread(buffer, 1, BYTES, file);
        *content = realloc(*content, *length + block);
        ...
        memcpy(*content + *length, buffer, block);
        *length += block;
    } while ( block!=0 );
...
const char* lookup(const char* path)
{
    ...
    char ext[strlen(temp)];
    strcpy(ext, temp+1);
    
    // text/css for any .css file
    if (strcasecmp("css", ext) == 0) return "text/css";
    
    // text/html for any .html file
    if (strcasecmp("html", ext) == 0) return "text/html";
    
    // image/gif for any .gif file
    if (strcasecmp("gif", ext) == 0) return "image/gif";
    
    // image/x-ico for any .ico file
    if (strcasecmp("ico", ext) == 0) return "image/x-icon";
    
    // image/jpeg for any .jpg file
    if (strcasecmp("jpg", ext) == 0) return "image/jpeg";
    
    // text/javascript for any .js file
    if (strcasecmp("js", ext) == 0) return "text/javascript";
    
    // text/x-php for any .php file
    if (strcasecmp("php", ext) == 0) return "text/x-php";
    
    // image/png for any .png file
    if (strcasecmp("png", ext) == 0) return "image/png";
    else return NULL;
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    const char* lineHighstack = line;
    const char* lineMethod = strstr(lineHighstack, " ");
    if (lineMethod == NULL)
    {
        error(500);
    }
    
    char method[lineMethod - lineHighstack + 1];
    strncpy(method, lineHighstack, lineMethod - lineHighstack);
    method[lineMethod - lineHighstack] = '\0';
    ...
    const char* firstSP = strchr(lineHighstack, ' ');
    const char* secondSP = strrchr(lineHighstack, ' ');
    
    char requestTarget[strlen(firstSP) - strlen(secondSP)];
    strncpy(requestTarget, firstSP + 1, strlen(firstSP) - strlen(secondSP));
    requestTarget[strlen(firstSP) - strlen(secondSP) - 1] = '\0';
    ...
    // if requestTarget doesn't contain '?', absolutePath = requestTarget, querry = "\0"
    if (strchr(requestTarget, '?') == NULL)
    {
        // absolutePath
        char absolutePath[strlen(requestTarget) + 1];
        strcpy(absolutePath, requestTarget);
        absolutePath[strlen(requestTarget)] = '\0';
        // printf("absolutePath: \t[%s], %lu\n", absolutePath, strlen(absolutePath));
        
        // querry
        char queryCopy[1];
        queryCopy[0] = '\0';
        // printf("queryCopy: \t[%s], %lu\n", queryCopy, strlen(queryCopy));
        
        // copy to abs_path and query
        strcpy(abs_path, absolutePath);
        abs_path[strlen(absolutePath)] = '\0';
        
        strcpy(query, queryCopy);
        query[strlen(queryCopy)] = '\0';
        // printf("abs_path:[%lu] \t[%s]\n", strlen(abs_path), abs_path);
        // printf("query:[%lu] \t[%s]\n", strlen(query), query);
    }else
    {
        // absolutePath
        const char* questionMark = strrchr(requestTarget, '?');
        char absolutePath[strlen(requestTarget) - strlen(questionMark) + 1];
        strncpy(absolutePath, requestTarget, strlen(requestTarget) - strlen(questionMark));
        absolutePath[strlen(requestTarget) - strlen(questionMark)] = '\0';
        // printf("absolutePath:[%lu] \t[%s]\n", strlen(absolutePath), absolutePath);
        
        // querry
        char queryCopy[strlen(questionMark) + 1];
        strcpy(queryCopy, questionMark + 1);
        queryCopy[strlen(queryCopy)] = '\0';
        // printf("queryCopy:[%lu] \t[%s]\n", strlen(queryCopy), queryCopy);
    ...
    // http version
    char httpVer[strlen(secondSP) - 2]; // without last symbols \r\n
    strncpy(httpVer, secondSP + 1, (strlen(secondSP) - 3));
    httpVer[strlen(secondSP) - 3] = '\0';
    //printf("httpVer:[%lu] [%s]\n", strlen(secondSP), httpVer);
